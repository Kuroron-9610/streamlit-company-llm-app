...
logger.info("ログ出力テスト：components.py から")
    ...
    for message in st.session_state.messages:
        ...
        with st.chat_message(message["role"]):
            ...
            # LLMからの回答の場合
                ...
                logger.info(f"回答データの型: {type(message['content'])}")
                logger.info(f"回答データの中身: {message['content']}")
...
def display_intro_message():
    """
    会話ログの上に表示する初期案内メッセージ
    """
    with st.chat_message("assistant"):
            st.success("こんにちは。私は社内文書の情報をもとに回答する生成AIチャットボットです。サイドバーで利用目的を選択し、画面下部のチャット欄からメッセージを送信してください。")
            st.warning("※社内文書の情報をもとに回答するため、正確な回答が得られない場合があります。")


def display_full_layout():
    """
    アプリ全体のレイアウトを構築（サイドバー＋メイン構成）
    """
    # スタイルの設定（全体幅＋サイドバー装飾）
    st.markdown(
        """
    ...
    chat_message = st.chat_input(ct.CHAT_INPUT_HELPER_TEXT, key="main_chat_input")
    return chat_message
